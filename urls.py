...
urlpatterns += patterns('',
    ...
    url (r'^users/tags/(?P<user_id>\d+)/$',
        ...
        name='user_tag_list'),
    url (r'^user/(?P<tag_name>[^^]*?)--(?P<tag_id>[-\w]+)/(?P<user_id>\d+)/$',
    ...
    url (r'^user/(?P<language>[-\w]+)/(?P<tag_name>[^^]*?)--(?P<tag_id>[-\w]+)/(?P<user_id>\d+)/$',
